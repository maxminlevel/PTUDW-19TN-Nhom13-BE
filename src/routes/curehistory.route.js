...
const CureHistoryService = require('@/services/curehistory.service')
...
router.get('/', authManager, midleware.page.request, async (req, res, next) => {
  try {
    res.success(await CureHistoryService.list(req.ctx, req.ctx.get('data')), {
...
router.get('/:id', authManager, async (req, res, next) => {
  try {
    res.success(await CureHistoryService.getUser(req.ctx, {id: req.params.id}))
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
  next()
...
router.post('/', authManager, async (req, res, next) => {
  try {
    res.success(await CureHistoryService.create(req.ctx, req.body))
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
  next()
...
router.put('/:id', authUser, async (req, res, next) => {
  try {
    ...
    res.success(await CureHistoryService.update(req.ctx, userId, req.body))
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
  next()
...
router.delete('/:id', authManager, async (req, res, next) => {
  try {
    const userId = req.params.id
    res.success(await CureHistoryService.remove(req.ctx, userId))
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
  next()
