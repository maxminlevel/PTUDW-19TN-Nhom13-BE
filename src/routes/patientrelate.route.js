...
const PatientRelateService = require('@/services/patientrelate.service')
...
router.get('/', authManager, midleware.page.request, async (req, res, next) => {
  try {
    res.success(await PatientRelateService.list(req.ctx, req.ctx.get('data')), {
...
router.get('/:id', authManager, async (req, res, next) => {
  try {
    res.success(
      await PatientRelateService.getUser(req.ctx, {id: req.params.id})
    )
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
...
router.post('/', authManager, async (req, res, next) => {
  try {
    res.success(await PatientRelateService.create(req.ctx, req.body))
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
  next()
...
router.put('/:id', authManager, async (req, res, next) => {
  try {
    ...
    res.success(await PatientRelateService.update(req.ctx, userId, req.body))
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
  next()
...
router.delete('/:id', authManager, async (req, res, next) => {
  try {
    const userId = req.params.id
    res.success(await PatientRelateService.remove(req.ctx, userId))
  } catch (error) {
    res.fail(400, error.detail, error.statusCode)
  }
  next()
